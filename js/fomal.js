...
$.ajax({
  ...
  data: {
    key: 'VCABZ-KB2WQ-KTP53-BYAXZ-762N3-IYFEY',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(113.345026, 23.157582, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function createtime1() {
  var grt = new Date("05/11/2023 11:15:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Cielocanto %c 你正在访问Ezaviaの梦境",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Cielocanto") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Cielocanto的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("05/11/2023 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ?`<img class='boardsign' src='https://img.shields.io/badge/Ceno-%E6%98%BC%E9%97%B4%E4%B8%8A%E8%AF%BE%E4%B8%AD-blue?style=social&logo=appveyor' title='余额减少的日子好难熬~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://img.shields.io/badge/Ceno-%E5%A4%9C%E6%99%9A%E6%94%BE%E5%B7%A5%E6%97%B6-blue?style=social&logo=appveyor' title='夜晚，该好好的休息喽~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://cielocanto.cc/img/top.webp);
    --darkmode-bg:url(https://cielocanto.cc/img/topnight.webp);
    --mobileday-bg: url(https://cielocanto.cc/img/mobile.webp);
    --mobilenight-bg: url(https://cielocanto.cc/img/mobilenight.webp);
  }`;
}
// 切换背景主函数
function changeBg(s) {
